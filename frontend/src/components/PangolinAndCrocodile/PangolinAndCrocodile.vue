...
<template>
  <div class="">
    <div id="">
      <header class="">
        <h1 class="">Pangolin and Crocodile Gallery</h1>
        <p class="">Scroll down to explore the collection</p>
      </header>
      <section v-for="(image, index) in images" :key="index" class="" :class="{'md:flex-row-reverse': index % 2 !== 0}"><div class="">
          <img :src="image.src" :alt="image.alt" :class="['pangolin-image', index === 0 ? 'SpinPango' : '']"/>
        </div>
